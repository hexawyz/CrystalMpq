...
namespace CrystalMpq.DataFormats
{
		...
		where T : struct
	{
		...
		private static DynamicMethod BuildReadRecordMethod(DbcHeader header, int localeFieldIndex, int localeFieldCount)
		{
			...
			// We should have found the same field count as indicated by the header
			// If not, throw an exception because this is not safe
			if (fieldCount != header.FieldCount)
				throw new InvalidCastException();
			// At the end, field offset should be equal to the size of one record
			// If not the case, then throw an exception
			if (fieldOffset != header.RecordSize)
				throw new InvalidCastException();
