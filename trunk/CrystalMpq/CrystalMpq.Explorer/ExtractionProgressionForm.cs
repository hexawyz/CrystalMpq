...
namespace CrystalMpq.Explorer
{
	...
	{
		// TODO: Evaluate the migration to Framework 3.5
		private delegate void Action();
		private delegate void Action<T>(T t);
		private delegate R Func<R, T, U, V>(T t, U u, V v);
		...
		{
			if (InvokeRequired) return (DialogResult)Invoke((Func<DialogResult, string, string, ulong>)AskForOverwrite, file, directory, newSize);
